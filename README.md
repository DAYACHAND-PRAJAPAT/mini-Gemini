...
Mini-Gemini is an open-source **agentic AI chatbot** built with **Python, FastAPI, LangGraph, LangChain, Google Gemini, Tavily, ChromaDB, and SQLite**.
...
```bash
git clone https://github.com/entbappy/Mini-Gemini.git
```
...
```bash
conda create -n Mini-Gemini python=3.11 -y
```

### 4. Activate the virtual environment

```bash
conda activate Mini-Gemini
```
...
```text
315865595366.dkr.ecr.us-east-1.amazonaws.com/Mini-Gemini
```

For GitHub Secrets, only save the repository name:

```text
ECR_REPO=Mini-Gemini
```
